...
public class UserSecretVerifier extends SecretVerifier {
	...
	public boolean verify(String identifier, char[] secret)
		...
		try {
			TypedQuery<User> query = em.createQuery("SELECT u FROM User u WHERE u.identifier = ?1", User.class);
			query.setParameter(1, identifier);
//			query.setParameter(2, secret);
			List<User> users = query.getResultList();
			return (users.get(0).getIdentifier().equals(identifier) && Arrays.equals(users.get(0).getSecret(), secret));
		} catch (Exception e) {
			e.printStackTrace();
		} finally {
			em.close();
